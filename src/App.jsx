          ...
          <>
            ...
            <nav style={{padding:"8px 12px",flex:1}}>
              {[{id:"board",icon:"⊞",label:"Tablero"},{id:"calendar",icon:"📅",label:"Calendario"},{id:"members",icon:"◎",label:"Participantes"}].map(item=>(
                <button key={item.id} onClick={()=>setView(item.id)} style={{width:"100%",padding:"9px 12px",borderRadius:8,border:"none",background:view===item.id?"rgba(245,158,11,0.12)":"transparent",color:view===item.id?"#f59e0b":"#64748b",cursor:"pointer",textAlign:"left",fontSize:13,fontWeight:view===item.id?700:400,display:"flex",alignItems:"center",gap:10,marginBottom:3}}>
                  <span style={{fontSize:14}}>{item.icon}</span>{item.label}
                </button>
              ))}
              <button onClick={()=>{setEditingProject(currentProject);setShowNewProject(true);}} style={{width:"100%",padding:"9px 12px",borderRadius:8,border:"none",background:"transparent",color:"#475569",cursor:"pointer",textAlign:"left",fontSize:13,display:"flex",alignItems:"center",gap:10,marginTop:4}}>
                <span style={{fontSize:14}}>⚙</span>Editar proyecto
              </button>
            </nav>
            <div style={{padding:"14px 16px",borderTop:"1px solid rgba(255,255,255,0.06)"}}>
              <div style={{fontSize:10,fontWeight:700,color:"#334155",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:10}}>Resumen</div>
